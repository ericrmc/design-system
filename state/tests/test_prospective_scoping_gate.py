...
def _seed_substantive_dv() -> str:
    ...
    nonce = m.group(1)
    sess_alias = _open_session_alias()
    res = _run_cli([
...
def test_substantive_session_refused_without_receipt(clean_substrate):
...
def test_nil_attestation_requires_exclusions_applied(clean_substrate):
...
def test_substantive_nil_must_name_implication_pattern(clean_substrate):
...
def test_substantive_nil_with_implication_pattern_admits(clean_substrate):
...
def test_prefix_case_insensitive(clean_substrate):
...
def test_prefix_with_leading_whitespace(clean_substrate):
...
def test_spec_version_makes_session_substantive(clean_substrate):
    ...
    env["SELVEDGE_WORKSPACE"] = str(PRIMARY_DB.parent.parent)
    res = subprocess.run(
    ...
    spec_file = PRIMARY_DB.parent.parent / "specifications" / "test-spec-fixture.md"
    if spec_file.exists():
        spec_file.unlink()


def test_non_nil_body_bypasses_nil_guards(clean_substrate):
...
def test_multi_row_first_fails_second_passes(clean_substrate):
