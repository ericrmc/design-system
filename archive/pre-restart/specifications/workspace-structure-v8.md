...
supersedes: workspace-structure-v7.md
superseded-by: workspace-structure.md (v9)
superseded-in-session: 064
---
...
Version 8 (Session 063 per D-228) extends §10.4 with five new first-class minorities §10.4-M16 through §10.4-M20 (S062 EF-058-tier-2-validation MAD dissent-preservation; minority count 40 → 45). Adds `validation-debt/` directory to top-level structure as engine-adjacent lightweight ledger per `validation-approach.md` v6 §(z5) Validation-Debt Lifecycle. Minor per OI-002 (additions only; no removals; no revisions to existing text); engine-v bump driven by `validation-approach.md` v6 substantive revision per `engine-manifest.md` engine-v11 entry, not by this file. v7 preserved as `workspace-structure-v7.md`.
