...
**Precedent-sensitive provenance check (recommended; cites OI-S114-1, DV-S116-1, DV-S117-1, DV-S173-1, DV-S174-1, DV-S175-1).** Before submitting a `decision-record` whose target re-proposes, re-traverses, or re-supersedes a prior decision/spec/issue surface, run the anchor-trace chain-walk against the cited precedent and read the result alongside the alternatives draft. The discipline is reading prior provenance to avoid re-deciding what the substrate already deliberated, not running a tool as ceremony — the clause is named after the property it defends (provenance-coherence) so future ergonomic shapes (e.g. the deferred FR-S173-4 batch) substitute without spec-version churn. A chain-walk that surfaces no precedent is a fast no-op; one that surfaces a prior rejection is the leverage. P-2 cross-family D-26 framing: the check is precedent-sensitive provenance, not run-anchor-trace.

**Invocation points.** Run the chain-walk at decision-record draft time when (a) `target_kind` is `spec_version` and `supersedes` cites a prior `SPEC-<spec>-v<n>` (the spec's own history is the precedent surface); (b) any `alternatives.rejections.basis` is `prior_decision` (the rejection is asserting a prior call applies — verify the prior actually applies); (c) the record's title or supports cite a `DV-S<wno>-<n>` whose own alternatives include a position close to the current adopt-shape (re-proposal risk per "Don't silently re-propose what was rejected before"); (d) the record reopens a closed issue, closed FR, or sealed deliberation. Outside these points, the check is optional.

**CLI surface.** Use `bin/selvedge export --provenance --anchor <alias> --max-depth <N> --print` for tactical reads — the `--print` mode emits the markdown body to stdout with no JSON wrapper and no disk write (FR-S173-1, DV-S174-1). `--max-depth` defaults to 3 (hard cap 5); prefer 2 for tight precedent reads (one supersedes hop plus its supports), 3 for cross-decision walks (the calibrated default for spec-version supersession chains), 5 only when the precedent surface is wide and the chain is sparse. Use `--write` (materialising to `provenance/anchor-traces/<alias>.md`) only when the trace is itself load-bearing for the record being authored, not for session-local reads.

**Authority and recovery (cites DV-S109-1 ceremony-subtraction, DV-S173-1 deliberation-shaped shipping, M-1 D-26).** This clause is recommended, not gated. The substrate does not refuse `decision-record` on chain-walk absence at engine-v47; the operator/agent invokes when an invocation point matches. Calibration-EFs are the recovery path when a downstream session reads a record and notices it re-proposed a previously-rejected option without citing the prior rejection (mirrors temporal-claim grounding and §8.5 close-time audit). P-3 D-26 minority preserved: 56-session non-use of anchor-trace post-S117 is read as natural-experiment evidence the named failure modes have not surfaced as calibration-EFs at intervention-warranting rates; FR-S173-3 is the calibration-watch tracking tactical invocation across S175..S179 to test that reading empirically.

**Promotion trigger to substrate gate (cites FR-S173-5, DV-S152-1).** If a future calibration-EF names a substantive decision that re-proposed a rejected alternative AND a chain-walk against a matching invocation-point alias would have surfaced the prior rejection, the next session opens a gate-promotion `OI` toward T-NN refusing `decision-record` submission on missing chain-walk evidence for invocation-point matches. The typed-observation→gate progression follows DV-S152-1's typed-conflict-primitive precedent; the calibration-EF is the recurrence signal, not first-instance.
