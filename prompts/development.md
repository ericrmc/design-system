...
**Operator-absent default — prefer tactical scope (cites EF-S165-1, EF-S167-1, FR-S165-9).** Under bare-prompt auto-proceed, when priority-2 (untriaged engine-feedback) or priority-3 (undisposed forward-reference) yields a tractable tactical path — a single-clause spec edit, an FR-disposition-only session, an EF triage-only meta session, or a named-FR closure that the prior session explicitly scoped as tactical — prefer it over a substantive path. Substantive scope under operator absence requires a clear named mandate (the prior session's `next_session_should` explicitly proposes substantive work, or a HIGH-priority open-issue is in queue); the auto-proceed agent does not resolve close substantive-vs-tactical judgment calls alone. If both paths have similar yield, prefer tactical — the operator returns and ratifies the substantive direction at the next operator-present session. S165 and S167 both reached tactical via priority-2 leverage; this clause names the default explicitly so future auto-proceed sessions do not re-derive it from priority-order alone.
