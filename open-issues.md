...
**Surfaced:** Session 001
**Status:** Open — narrowed-pending-sustained-practice after Session 005 (phrased as tally, not endorsement); tally advanced from 1 of 3 to 2 of 3 in Session 006 per D-043; tally **unchanged at 2 of 3** after Session 007 per D-049; tally **unchanged at 2 of 3** after Session 008 per D-052 (non-Claude participation included but no D-023 trigger fired; second consecutive non-advancing non-Claude session)
...
A future session may consider whether criterion 2 should expand to count voluntary non-Claude inclusion (i.e., sessions where non-Claude participation occurs without being D-023-required); Session 007 explicitly considered and rejected opening an OI for this question per the "wait for a concrete pattern" principle; Session 008 extends the pattern to two consecutive instances but does not revisit the question. If Session 009+ accumulates a third voluntary-inclusion-no-tally-advance instance, the question may be re-opened.
...
**Surfaced:** Session 001
**Status:** Open — no longer blocked on first external application (Session 008 executed the first external application producing a movement-sequence artefact)
The nine core activities may need sub-activities, and different types of work (research, design decisions, validation, implementation) may warrant different subsets or emphases. Session 008's Produce activity surfaced four watchpoint findings (W1–W4 per D-051) that are relevant here: W1 (Read activity for external domains has two senses — workspace-reading vs. domain-knowledge absorption) and W4 (Validate activity for external domains has two senses — internal methodology-consistency vs. domain-native testing) are work-type-variant observations that would inform a sub-activities deliberation. Full OI-005 deliberation deferred until a **second** external application exists (to avoid proposing sub-activity variants from one data point); Session 009+ may activate.
...
**Surfaced:** Session 001
**Status:** Open
Open issues are currently a single file. After Session 008 the count remains 9 open issues (no new OIs opened this session; Session 008 considered and rejected opening OI-012 for external-artefact-placement-standard and OI-013 for external-vs-internal-Validate-senses, both per the "wait for a concrete pattern" principle inherited from Session 006 D-043 and Session 007 D-049 — Session 008's W2 and W4 watchpoint findings in D-051 capture these concerns without OI overhead). The file remains readable. Consider migrating to a directory of individual issue files, or developing a categorization system, when the need arises.
...
**Status:** **Monitor — first external application executed Session 008.** Conditional escalation from Session 007 D-048 resolved per D-052: Session 008 executed first external application (Candidate 3 movement sequence, user-ratified, artefact committed at `provenance/008-first-external-application/artefact-morning-unfurl.md`), so the D-048.4 methodology-claim-downgrade provision does not activate. Continued monitoring via the G/O/K/S criterion-package adopted in Session 007 D-048.2.
