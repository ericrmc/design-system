...
# Sources: constraints.md v1 (retired at DV-S109-1) named the six failure
# properties; EF-S161-1 named the kernel-narrative gap that retirement
# created (substrate refusals carry the mechanism, this distillation carries
# the narrative the agent needs at decision-time). S075/S076/S078 carry the
# foundational provenance; S162 EF-S162-1 vindicated the external-pilot-plus-
# harvest pathway. Edit deliberately; this is read every session-open.
_WHY_THIS_ENGINE_EXISTS = """\
Selvedge defends against six observed failure modes of LLM-agent reasoning:

- Loses foundational instructions under context pressure.
- Anchors on most-recent-prior-decision over foundational specification.
- Confuses authority of speakers (operator, prior session, recent message).
- Generates narrative-fitting numbers when substrate values exist.
- Defaults to mechanism-addition over mechanism-subtraction.
- Drifts into ceremony when each addition does not pay for itself.

The kernel encodes these at the substrate layer (typed rows, T-NN refusals,
mandatory close-time audit + seal-grading, FR/EF disposition discipline).
constraints.md v1 named the properties before retirement at DV-S109-1; the
narrative is preserved here so the agent recognises which refusal-shape
would have applied at decision-time (cites EF-S161-1).

Recursive self-application: the engine's own development applies the engine.
Methodology-changing decisions convene cross-family perspectives. The
S151/S152/S154/S162 disaster-response pilot validated the
external-pilot-plus-harvest pathway — external pressure surfaces what
self-development cannot simulate; harvest sessions convert raw signal to
OIs and decisions (cites EF-S162-1).

Foundational provenance: `archive/pre-restart/` (S075 problem statement,
S076 restart, S078 design commitments). Read these when a
methodology-changing decision is on the table.
"""
...
def _orient_sections(conn: sqlite3.Connection) -> dict:
    ...
    """
    out: dict = {}
    out["why_this_engine_exists"] = _WHY_THIS_ENGINE_EXISTS
    out["workspace"] = {
        r["key"]: r["value"]
...
def _orient_markdown(packet: dict) -> str:
    ...
    why = packet.get("why_this_engine_exists")
    if why:
        lines.append("## Why this engine exists")
        lines.append("")
        lines.append(why.rstrip())
        lines.append("")
    ws = packet["workspace"]
