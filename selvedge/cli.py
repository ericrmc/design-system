...
def _anchor_synthetic_alias(node: tuple) -> str:
    ...
    Returns the canonical alias if present (e.g. DV-S116-1, OI-S114-1, S116);
    otherwise FR-S<wno>-<seq> for forward_references and DELIB-<id> for
    deliberations. Falls back to <kind>:<pk> for any unaliased kind we still
    encounter (defence-in-depth; should not occur within the closed family).
    """
    kind, pk, alias = node
    if alias:
        return alias
...
def _node_from_kind_pk(conn: sqlite3.Connection, kind: str, pk: int) -> Optional[tuple]:
    ...
    Aliases live in `objects.alias` for typed kinds and `issues.alias` for
    issues. Sessions don't carry an alias column on objects; synthesised as
    S<NNN> via _node_from_session_id. Deliberations don't have aliases at
    all; synthesised as DELIB-<id> at display time.
    """
    if kind == "session":
...
def _resolve_anchor(conn: sqlite3.Connection, alias: str) -> tuple:
    ...
    objects table. Issues are checked before the generic objects lookup
    because OI- aliases live in `issues.alias`, not `objects.alias`.
    """
    ...
    if m:
        did = int(m.group(1))
        row = conn.execute("SELECT deliberation_id FROM deliberations WHERE deliberation_id=?", (did,)).fetchone()
        if row is None:
...
def _decisions_citing_object(conn: sqlite3.Connection, oid: int) -> list[tuple]:
    ...
    references this object_id via the closed edge family. Deterministic order.
    """
    out: list[tuple] = []
...
def _decisions_citing_issue(conn: sqlite3.Connection, issue_id: int) -> list[tuple]:
    ...
    and (b) target_descriptor substring match for legacy `opens_issue` /
    `closes_issue` effects whose FK was never populated. Migration 021
    backfilled most resolvable cases but `opens_issue` effects often carry
    descriptors that name a not-yet-created alias, so the FK can stay NULL.
    Without the descriptor fallback, anchor-trace from an issue would miss
    its originating decision.
    """
    seen: set[tuple] = set()
    out: list[tuple] = []
    for e in conn.execute(
    ...
    if alias_row and alias_row["alias"]:
        ...
        alias_pat = f"%{alias}%"
        # SQL LIKE is a substring match; OI-S110-1 would also match a
        # descriptor mentioning OI-S110-12. Word-boundary regex narrows
...
def _anchor_neighbours(conn: sqlite3.Connection, node: tuple) -> list[tuple]:
    ...
    Direction is 'out' when this node is the source of the edge, 'in' when
    the neighbour points at this node. Neighbours are deterministic in
    enqueue order: edges within a single source row preserve their seq /
    rowid; the caller batches by edge_kind for the markdown projection.
    """
    kind, pk, _alias = node
    edges: list[tuple] = []
    ...
    if kind == "decision_v2":
        ...
        if p:
            edges.append(("in_deliberation", "out", _node_from_kind_pk(conn, "deliberation", p["deliberation_id"])))
        ...
        if d:
            ...
            for r in conn.execute(
                ...
                edges.append(("supports[deliberation]", "in", _node_from_kind_pk(conn, "decision_v2", r["decision_v2_id"])))
        for r in conn.execute(
            "SELECT perspective_id FROM perspectives WHERE deliberation_id=? ORDER BY perspective_id",
            (pk,),
        ).fetchall():
            edges.append(("has_perspective", "out", _node_from_kind_pk(conn, "perspective", r["perspective_id"])))

    elif kind == "session":
        # Session is a leaf-ish node in the closed edge family per DV-S116-1.
        # Anchoring on a session is supported via _expand_session_root; when a
        # session is reached as a *neighbour* (e.g. surfaced_in from an EF, or
        # disposed_in from a forward-reference disposition) we deliberately do
        # NOT broadcast to every sibling decision/EF/spec — that would dilute
        # the causal trace by pulling in artefacts unrelated to the chain.
        # The session row carries enough metadata (slug, kind, mode) for
        # _anchor_node_summary to render usefully as a leaf.
        return edges
        ...
        # state_item_id → close_record/session
        r = conn.execute(
        ...
        # Surface the parent decision and any cited rejections. Alternatives
        # rarely become anchors directly, but the closed family includes them.
        r = conn.execute(
...
def _export_provenance_anchor(
    ...
    def neighbours_for(node: tuple, is_root: bool) -> list[tuple]:
        ...
        if is_root and node[0] == "session":
            ...
            for r in conn.execute(
                ...
                edges.append(("deliberated_in", "out", _node_from_kind_pk(conn, "deliberation", r["deliberation_id"])))
            for r in conn.execute(
    ...
    while frontier:
        ...
        for depth, node in frontier:
            if depth >= max_depth:
                # Edges out of the boundary are surfaced for transparency but
                # not expanded — the trace says "stops here, raise --max-depth
                # to widen" rather than silently dropping.
                continue
        ...
        # Deterministic enqueue: sort next frontier by display alias so two
        # runs over identical substrate emit identical markdown.
        next_frontier.sort(key=lambda dn: display(dn[1]))
        frontier = next_frontier

    # Resolve display aliases now that all neighbours are fixed (some neighbour
    # tuples were stored with alias=None to defer the lookup).
    ...
    # Boundary surface: list edges from the cap frontier that the trace
    # did not follow further. The cap frontier is depth==max_depth nodes;
    # their outbound edges are not enumerated, so we surface a count rather
    # than individual edges. Operators raise --max-depth to expand.
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p_exp.add_argument("--max-depth", dest="max_depth", type=int, default=None,
